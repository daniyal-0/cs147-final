...
void loop() 
{
  ...
  Serial.println("% rH");
  fan_speed = int((double(temp.temperature) - max_temp) / (double(min_temp) - max_temp) * max_speed);
...
// UNCOMMENT:
   ...
   WiFiClient c;
   HttpClient http(c);
   std::string terminal = "/?var=temp:" + std::to_string(temp.temperature) + "humidity:" + std::to_string(humidity.relative_humidity);
   err = http.get("3.138.100.131", 5000, terminal.c_str(), NULL);
   if (err == 0)
   {
  ...
  delay(1000); // 1-second delay between each speed reduction
}
